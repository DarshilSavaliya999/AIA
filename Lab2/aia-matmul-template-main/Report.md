...
| Property | Value |
|---|---|
| CPU model | AMD Ryzon 5 5600H with Radeon Graphics|
| Number of cores / threads | 6 cores / 12 threads|
| Base / Boost clock speed (GHz) | 3.30 GHz|
| SIMD ISA (SSE4.2 / AVX2 / AVX-512 …) | AVX2 |
| SIMD width (bits / floats per vector) | 256-bit/8 floats |
| MAC units per core | 2 (approximate) |
| L1 cache size (per core) | 384KB|
| L2 cache size (per core) | 3.0MB|
| L3 cache size (shared) | 16.0MB|
| Peak theoretical throughput (GFLOP/s) | ~317 GFLOPS |

**How did you calculate peak throughput?**

_(formula: cores × clock × SIMD_width × MACs_per_cycle)_

= 6 cores × 3.30 GHz × 8 floats × 2
= 316.8 GFLOPS
---
